...
def buscar_reservas_cliente(db, tipo_db):
    ...
    try:
        ...
        if tipo_db == "oracle":
            sentencia = "SELECT c.nombre, r.fecha_entrada, h.numero FROM clientes c JOIN reservas r ON c.id_cliente = r.id_cliente JOIN habitaciones h ON r.id_habitacion = h.id_habitacion WHERE c.dni = :dni"
            cursor.execute(sentencia, {'dni': dnibusqueda})
        else:
            sentencia = "SELECT c.nombre, r.fecha_entrada, h.numero FROM clientes c JOIN reservas r ON c.id_cliente = r.id_cliente JOIN habitaciones h ON r.id_habitacion = h.id_habitacion WHERE c.dni = '%s'" % dnibusqueda
            cursor.execute(sentencia)
